...
STARTUP_ID = f"{str(uuid.uuid4())[:8]}_fix_reassigned_night_shift_dates"
...
# Importar routers
from backend.routers import empleados, sync, turnos, asistencia, configuracion, reportes, dashboard_api, startup, auth, seguridad, cierre, productos_4, porteria, articulo22, visitas, llaves, flota
...
# CORS Middleware (Permitir cualquier origen local o puerto como http://localhost:8099)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
app.include_router(productos_4.router, prefix="/api", tags=["4 Productos"])
...
app.include_router(flota.router, prefix="/api", tags=["Flota Aguacol"])
