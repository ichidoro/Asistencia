...
async def get_estado_dia(
    ...
    # Grupar marcas por flota_id (ya vienen ordenadas por r.id ASC, que es cronológico)
    marcas_por_vehiculo = {}
    for m in marcas:
        fid = m["flota_id"]
    ...
    for veh in vehiculos:
        ...
        # Procesar pares de marcas cronológicamente ordenados por ID
        i = 0
        ...
        # Evaluar sesión activa (si la fecha consultada es hoy)
        ultima_marca = veh_marcas[-1] if veh_marcas else None
        
        if es_hoy and ultima_marca:
            # Calcular tiempo acumulado en curso hasta 'ahora'
            dt_ultima = dts[-1]
            diff_activo = (now - dt_ultima).total_seconds() / 60.0
