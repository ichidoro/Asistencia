...
async def rebuild_and_recalc():
    ...
    print(f"   -> {len(employees)} empleados encontrados.", flush=True)
    ...
    print("6. Iniciando reprocesamiento de asistencia desde 2026-04-26 hasta 2026-06-02...", flush=True)
    for idx, emp in enumerate(employees, 1):
        emp_id = emp['id']
        emp_name = emp['nombre']
        print(f"   [{idx}/{len(employees)}] Procesando {emp_name} (ID: {emp_id})...", flush=True)
        try:
            # We call reprocesar_periodo_empleado, which computes and saves to DB.
...
if __name__ == "__main__":
    asyncio.run(rebuild_and_recalc())
