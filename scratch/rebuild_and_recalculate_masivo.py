...
async def rebuild_and_recalc_masivo():
    ...
    print("5. Iniciando reprocesamiento de asistencia masivo de todos los empleados desde 2026-04-26 hasta 2026-06-02...", flush=True)
    ...
    t0 = datetime.now()
    res = await asist_service.reproceso_masivo_async(
        fecha_inicio="2026-04-26",
        fecha_fin="2026-06-02",
        force=True
    )
    t_diff = (datetime.now() - t0).total_seconds()
    print(f"   -> Reprocesamiento masivo finalizado en {t_diff:.1f}s. Resultado: {res}", flush=True)

    # 6. Clean up corrupted tables to free database space if possible
    print("6. Eliminando tablas corruptas obsoletas...", flush=True)
    try:
    ...
    print("7. Ejecutando PRAGMA integrity_check final...", flush=True)
...
if __name__ == "__main__":
    asyncio.run(rebuild_and_recalc_masivo())
