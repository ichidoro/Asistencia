...
async def simulate_full_generation():
    print("[SIMULACION] Iniciando simulacion fisica de generacion del libro Excel...")
    
    # 1. Conexión a BD y obtención de datos reales
    db = Database()
    ...
    if not periodo_activo:
        ...
        if not periodos:
            print("[ERROR] No se encontraron periodos.")
            return
    ...
    feriados_dict = {f['fecha']: f.get('descripcion', 'Feriado') for f in feriados}
    ...
    # 2. Procesar cálculos de los empleados en Python
    rows_calc = []
    hay_bolsa = False
    ...
    for emp in empleados:
        ...
        if es_bolsa:
            ...
            for d in rango_dias:
                ...
                di_check = dias_dict = emp_matrix.get(str(emp_id), {}).get(f_str, {})
        ...
        rows_calc.append({
            ...
            "rut": emp.get("rut", ""),
            "activo": "SI" if emp.get("activo", 1) else "NO",
            "esBolsa": es_bolsa,
            "he_bruto": he_bruto,
            "he_apr": he_apr,
            "he_rec": he_rec,
    ...
    # 3. Inicializar openpyxl Workbook
    wb = Workbook()
    
    # Definir paleta de estilos corporativos
    ...
    for sheet_title, view_mode in pestañas:
        ...
        # Logo corporativo
        logo_path = os.path.join(str(base_path), "frontend", "assets", "img", "logo.jpg")
        if os.path.exists(logo_path):
            try:
                ...
                print(f"[WARN] No se cargo imagen logo: {ex}")
        ...
        ws['J2'] = f"{fecha_inicio} al {fecha_fin}"
            ...
            for col_idx, (_, _, key) in enumerate(columns_def, start=1):
                ...
                # Bonos
                    ...
                    if not b_res or b_res.get("aplica") is False:
                        ...
                        val = "SI" if b_res.get("califica") else "NO"
                        if val == "SI":
                ...
                # HE
                elif key == "he_pend": val = format_hhmmss(r["he_pend"])
                elif key == "he_apr": val = format_hhmmss(r["he_apr"])
                elif key == "he_rech": val = format_hhmmss(r["he_rec"])
                elif key == "he_tot": val = format_hhmmss(r["he_bruto"])
                # Deudas
                elif key == "min_col": val = format_hhmmss(r["min_col"])
                elif key == "min_per": val = format_hhmmss(r["min_per"])
                elif key == "min_atr": val = format_hhmmss(r["min_atr"])
                elif key == "min_sad": val = format_hhmmss(r["min_sad"])
                elif key == "d_tot": val = format_hhmmss(r["d_tot"])
                # Saldo Neto
                elif key == "saldo_neto":
                    val = get_saldo(r["he_apr"], r["d_tot"])
                    if "+" in val:
                        cell.font = Font(name="Segoe UI", size=9, color="008000", bold=True)
                    elif "-" in val:
                        cell.font = Font(name="Segoe UI", size=9, color="FF0000", bold=True)
                # Bolsa Flexible
                elif key == "meta_min": val = format_hhmmss(r["metaMin"]) if r["esBolsa"] else ""
                elif key == "acum_bolsa": val = format_hhmmss(r["acumBolsa"]) if r["esBolsa"] else ""
                elif key == "saldo_meta":
                    if r["esBolsa"]:
                        sm = r["saldoMeta"]
                        sm_sign = "+" if sm > 0 else ("-" if sm < 0 else "")
                        val = f"{sm_sign}{format_hhmmss(abs(sm))}"
                        if sm >= 0:
                            cell.font = Font(name="Segoe UI", size=9, color="008000", bold=True)
                        else:
                            cell.font = Font(name="Segoe UI", size=9, color="FF0000", bold=True)
                    ...
                    if di:
                        ...
                        # --- PESTAÑA HORAS REALES ---
                            ...
                            if di_estado in ['OK', 'ATRASO', 'SALIDA_ADELANTADA', 'ATR_SAD', 'JORNADA_ESPECIAL', 'EXTRA', 'EN_CURSO']:
                                if di_estado == 'EN_CURSO':
                                    ...
                                    hrs = format_hhmmss(trab)
                                    if not r["esBolsa"] and deuda > 0:
                                        val = f"{hrs}\nDEUDA {format_hhmmss(deuda)}"
                        ...
                        # --- PESTAÑA COLACIÓN ---
                            ...
                            if di_estado in ['OK', 'ATRASO', 'SALIDA_ADELANTADA', 'ATR_SAD', 'JORNADA_ESPECIAL', 'EXTRA', 'EN_CURSO']:
                                if di.get("hora_entrada_real") and di.get("hora_salida_real"):
                                    ...
                                    col_str = format_hhmmss(col_aplicado)
                                    if exceso > 0:
                                        val = f"{col_str}\nDEUDA {format_hhmmss(exceso)}"
                                        cell.font = Font(name="Segoe UI", size=8, color="FF0000")
                        ...
                        # --- PESTAÑA HORAS EXTRAS ---
                            ...
                            if r["esBolsa"]:
                                ...
                                txt = ""
                                if he_bruto_d > 0:
                                    txt += f"+{format_hhmmss(he_bruto_d)}"
                                if deuda_d > 0:
                                    txt += f"\n-{format_hhmmss(deuda_d)}"
                                val = txt.strip()
                                if "+" in val and "-" in val:
                                    cell.font = Font(name="Segoe UI", size=8) # multicolor rich text en excel requiere mas codigo, texto simple es robusto
                                elif "+" in val:
                                    cell.font = Font(name="Segoe UI", size=9, color="008000", bold=True)
                                elif "-" in val:
                                    cell.font = Font(name="Segoe UI", size=9, color="FF0000")
                        ...
                        # --- PESTAÑA ACUMULADO ---
                            ...
                            if r["esBolsa"]:
                                ...
                                if di_estado in ['OK', 'ATRASO', 'SALIDA_ADELANTADA', 'ATR_SAD', 'JORNADA_ESPECIAL', 'EXTRA', 'EN_CURSO'] and di.get("horas_trabajadas", 0) > 0:
                                    ...
                                    val = format_hhmmss(snap) if snap > 0 else "—"
                        
                        # Si cae en fin de semana sin marcas en conceptos
                        if not val and view_mode == "conceptos":
                            if di.get('es_fin_de_semana'):
                                val = "---"
                    else:
                        # Si no hay datos pero el turno dice que es libre estructural
                        es_libre_puro = False
                        if emp.get("turno_dias") and f_str:
                            py_day = d.getDay() if hasattr(d, 'getDay') else d.weekday()
                            # 0=Lunes en python
        ...
        for col_idx, (_, col_name, key) in enumerate(columns_def, start=1):
            ...
            # Totales de incidencias
                ...
                val = s_val if s_val > 0 else ""
            # Totales HE
            elif key == "he_pend": val = format_hhmmss(sum(r["he_pend"] for r in rows_calc))
            elif key == "he_apr": val = format_hhmmss(sum(r["he_apr"] for r in rows_calc))
            elif key == "he_rech": val = format_hhmmss(sum(r["he_rec"] for r in rows_calc))
            elif key == "he_tot": val = format_hhmmss(sum(r["he_bruto"] for r in rows_calc))
            # Totales Deudas
            elif key == "min_col": val = format_hhmmss(sum(r["min_col"] for r in rows_calc))
            elif key == "min_per": val = format_hhmmss(sum(r["min_per"] for r in rows_calc))
            elif key == "min_atr": val = format_hhmmss(sum(r["min_atr"] for r in rows_calc))
            elif key == "min_sad": val = format_hhmmss(sum(r["min_sad"] for r in rows_calc))
            elif key == "d_tot": val = format_hhmmss(sum(r["d_tot"] for r in rows_calc))
            # Total Saldo Neto
            elif key == "saldo_neto":
                s_tot = sum(r["he_apr"] - r["d_tot"] for r in rows_calc)
                val = ("+" if s_tot > 0 else "") + format_hhmmss(s_tot)
                if s_tot > 0:
                    cell.font = Font(name="Segoe UI", size=9, color="008000", bold=True)
                elif s_tot < 0:
                    cell.font = Font(name="Segoe UI", size=9, color="FF0000", bold=True)
            # Bolsa flexible totales no aplican en sumatoria directa
        ...
        foot_start += 1
        ws.merge_cells(f"B{foot_start}:J{foot_start}")
        foot_cell = ws.cell(row=foot_start, column=2, value=f"Generado automaticamente por el Sistema de Asistencia de Aguacol S.A. - {datetime.now().strftime('%d/%m/%Y')}")
        ...
        for col in ws.columns:
            ...
            for cell in col:
                # Ignorar pie de página para el ancho
    ...
    # Remover hoja por defecto creada al inicializar Workbook
    if "Sheet" in wb.sheetnames:
        wb.remove(wb["Sheet"])
        
    # Guardar en archivo
    out_file = Path("c:/Users/danie/Proyectos_Python/Asistencia/scratch/Reporte_Asistencia_Simulation.xlsx")
    wb.save(str(out_file))
    print(f"[EXITO] Libro Excel guardado en {out_file}")
    
    # Comprobar que el archivo se puede leer correctamente y que tiene las 6 hojas
    from openpyxl import load_workbook
    wb_test = load_workbook(str(out_file))
    print(f"[TEST] Hojas leidas del archivo generado: {wb_test.sheetnames}")
    for name in wb_test.sheetnames:
        ws_test = wb_test[name]
        print(f"  -> Hoja '{name}': {ws_test.max_row} filas x {ws_test.max_column} columnas")

if __name__ == "__main__":
    asyncio.run(simulate_full_generation())
